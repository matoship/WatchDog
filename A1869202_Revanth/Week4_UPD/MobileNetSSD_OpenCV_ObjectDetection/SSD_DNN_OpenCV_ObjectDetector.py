# CMD: python SSD_DNN_OpenCV_ObjectDetector.py --prototxt MobileNetSSD_deploy.prototxt.txt --model MobileNetSSD_deploy.caffemodel --image TestData/test3.jpeg
...
for i in np.arange(0, detections.shape[2]):
	...
	if confidence > args["confidence"]:
		...
		cv2.putText(image, label, (startX, y),
			cv2.FONT_HERSHEY_SIMPLEX, 0.8, COLORS[idx], 3)
		
# show the output image and save the file
cv2.imshow("Output", image)
cv2.imwrite("output/{}_output.jpg".format(args["image"].split("/")[1].split(".")[0]), image)
cv2.waitKey(0)

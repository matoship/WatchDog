...
def PreprocessingData(patients_videos_path:Path, patients_images_path:Path, desired_frame_count:int, train_data_path:Path, test_data_path:Path, train_test_split:Path, desired_shape:tuple = (224, 224, 3), training_model:bool = False):
    """
...
if __name__ == "__main__":
    ...
    ap.add_argument("-tm", "--training_model", required=False,
    ...
    PreprocessingData(Path(args["patients_videos_path"]), Path(args["patients_images_path"]), int(args["desired_frame_count"]), Path(args["train_data_path"]), Path(args["test_data_path"]), float(args["train_ratio"]), desired_shape=(int(args["desired_shape"]), int(args["desired_shape"]), 3), training_model=bool(int(args["training_model"])))

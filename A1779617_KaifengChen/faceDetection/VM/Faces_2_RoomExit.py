...
cred = credentials.Certificate(
    'watchdog-gamma-firebase-adminsdk-nbg3v-7e823e2078.json')
...
def data_extraction_thread():
    try:
        ...
        while True:
            ...
            if (new_data and (len(new_data) > 1 or last_processed_id is None)):
                for key, entry in new_data.items():
                    shared_data_queue.append(
                        (entry["face"], entry["distance"], entry["room_no"]))
        ...
        print(
            "[INFO] T1: Exiting data extraction thread (KeyboardInterrupt or unexpected error)..")
    ...
    try:
        ...
        while True:
            if shared_data_queue.__len__() > 0:
                ...
                face = tf.io.decode_image(
                    face, channels=3, dtype=tf.dtypes.uint8)
                ...
                matched_probability = float(predictor.Predictions(
                    room_no, face, model_name, target_size, Patient_images_folder))
                print(
                    f"[INFO] Room ID: {room_no}'patient is found on doorway, at a distance of {distance} matched with {matched_probability}%")
                ...
                if matched_probability > 50.0:
                    if room_no in processed_data_map:
                        ...
                        processed_data_map[room_no] = {
                            "prev_distance": distance, "prev_prev_distance": distance, "room_no": room_no, "room_exitted": None}
                    ...
                    patient_exitted_room = (
                        processed_data_map[room_no]["prev_distance"] < processed_data_map[room_no]["prev_prev_distance"])
                    ...
                    if ((processed_data_map[room_no]["room_exitted"] is None) or (processed_data_map[room_no]["room_exitted"] != patient_exitted_room)):
                        ...
                        room_exit_db.child(str(room_no)).set({"Room_Exited": patient_exitted_room, "Prev_distance": processed_data_map[
                            room_no]["prev_distance"], "Prev_Prev_distance": processed_data_map[room_no]["prev_prev_distance"]})
...
if __name__ == "__main__":
    ...
    t2 = Thread(target=data_processing_thread, args=(args["model_name"], (int(
        args["target_size"]), int(args["target_size"]), 3), args["Patient_images_folder"]))
